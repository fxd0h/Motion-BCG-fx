...
#define TIMESTEP_WAIT 100
#define TIMESTEP_INC 0.01
#define LIMIT_LOW -45
#define LIMIT_HIGH 45
...
//  SBGC32_ControlConfig(&SBGC32_Device, &ControlConfig, &Confirm);
  ...
  PrintBoardParameters(&SBGC32_Device);//, P_CURRENT_PROFILE);
}
...
double map ( double input,double input_start, double input_end, double output_start, double output_end) {
  ...
  if ((input < input_start)||(input>input_end)) return 0;
